...
# https://www.ebras.bio.br/dives/guide_dives.aspx?IDTopic=3E0E610F-FBF15B80-B1FA641C&Topico=ST15
# https://www.ebras.bio.br/dives/guide_dives.aspx?IDTopic=05E9C14F-FBF15B80-668EFE65&Topico=ST15
# https://www.quora.com/What-are-the-differences-between-Manhattan-Distance-and-Chebyshev-Distance#:~:text=As%20I%20understand%20it%2C%20both,the%20Manhattan%20Distance%20does%20not.
# https://en.wikipedia.org/wiki/Minkowski_distance

import heapq

# I have decided using Manhattan distance as the heuristic function
# because it looks more like the real distance between two points
# in a map. As we can think in the streets, roads and avenues as
# lines in a grid, and the distance between two points is the sum
# of the distances between the points in the x and y axis.
# This way, we won't underestimate the distance between two points
# which is the main problem of the Euclidean distance
#
# Furmula = Σ|Ai – Bi|
...
def shortest_path(map, start, goal):
    return _shortest_path(map.intersections, map.roads, start, goal)


def _shortest_path(intersections, roads, start, goal):
    open_list = [(manhattan_distance(intersections[start], intersections[goal]), start)]
    closed_list = set()
    ...
    while open_list:
        ...
        for neighbor in roads[current]:
            ...
            new_cost = cost[current] + manhattan_distance(intersections[current],
                                                          intersections[neighbor])

            if neighbor not in cost or new_cost < cost[neighbor]:
                cost[neighbor] = new_cost
                priority = new_cost + manhattan_distance(intersections[neighbor],
                                                         intersections[goal])
