...
class Graph:
    def __init__(self, node_list):
        self.nodes: list[GraphNode] = node_list
...
def dijkstra(graph: Graph, start_node: GraphNode, end_node: GraphNode):
    ...
    while distance_dict:
        ...
        smallest_node, distance = sorted(distance_dict.items(), key=lambda item: item[1])[0]

        # Remove the current node from the distance_dict, and store the same key:value in shortest_distance
        shortest_distance[smallest_node] = distance_dict.pop(smallest_node)

        # Check for each neighbour of current_node, if the distance_to_neighbour is smaller than the already
